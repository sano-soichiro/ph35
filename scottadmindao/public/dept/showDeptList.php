<?php
/**
 * PH35 サンプル5 マスターテーブル管理DAO版　Src06/12
 *  部門情報リスト表示。
 * 
 * @author Shinzo SAITO
 *
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= $dept->getDpLoc() ?></td>
                    <td>
                        <form action="/ph35/scottadmindao/public/dept/prepareDeptEdit.php" method="post"><input type="hidden" id="editDeptId<?= $dept->getId() ?>" name="editDeptId" value="<?= $dept->getId() ?>"><button type="submit">編集</button></form>
                    </td>
                    <td>
                        <form action="/ph35/scottadmindao/public/dept/confirmDeptDelete.php" method="post"><input type="hidden" id="deleteDeptId<?= $dept->getId() ?>" name="deleteDeptId" value="<?= $dept->getId() ?>"><button type="submit">削除</button></form>
                    </td>
                </tr>
                <?php
                        }
